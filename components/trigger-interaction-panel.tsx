...
export function TriggerInteractionPanel() {
  const { state, dispatch } = useAppContext();
  const { selectedTrigger } = state;
  
  const currentProject = state.projects.find(p => p.id === state.currentProjectId);
  const currentLevel = currentProject?.levels.find(l => l.id === selectedTrigger?.levelId);
  const triggerActions = currentLevel?.triggerActions?.[selectedTrigger?.triggerId ?? -1] ?? [];
  
  const [actions, setActions] = useState<ObjectAction[]>(triggerActions);

  useEffect(() => {
    setActions(currentLevel?.triggerActions?.[selectedTrigger?.triggerId ?? -1] ?? []);
  }, [selectedTrigger, currentLevel]);

  const handleUpdateActions = (newActions: ObjectAction[]) => {
    if (!selectedTrigger) return;
    setActions(newActions);
    dispatch({ type: 'UPDATE_TRIGGER_ACTION', payload: { ...selectedTrigger, actions: newActions } });
  }
  ...
  return (
    <div className="px-4 py-1 space-y-4">
      <div>
        <h3 className="font-semibold">Trigger #{selectedTrigger.triggerId} Actions</h3>
